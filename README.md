...
Update v1.1: you can now force load a chunk, by instead of just putting a `callback`, you can use the format `[callback,[x,y,z]]`, and it will make sure the chunk at x,y,z is loaded before executing callback, and it respects your order of tasks, but it looks ahead to the next tasks to load their chunks to save time
